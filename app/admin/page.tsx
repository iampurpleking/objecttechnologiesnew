  ...
  useEffect(() => {
    const fetchUsersAndAdmins = async () => {
      ...
      try {
        ...
        if (!adminsRes.ok)
          throw new Error(adminsJson.error || "Failed to fetch admins");
                ...
                return (
                  <tr key={user.id}>
                    ...
                    <td className="px-4 py-2">
                      {isAdmin ? adminObj.role : "-"}
                    </td>
                    <td className="px-4 py-2">
                      {isAdmin ? (
                        <button
                          onClick={() => demoteFromAdmin(adminObj)}
...
function DashboardOverview() {
  ...
  const recentActivity = [
    ...
    {
      type: "blog",
      message: 'New blog post published: "Next.js Best Practices"',
      time: "1 hour ago",
    },
    {
      type: "user",
      message: "User complaint submitted by sarah@example.com",
      time: "2 hours ago",
    },
  ];
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        const headers: HeadersInit = accessToken
          ? { Authorization: `Bearer ${accessToken}` }
          : {};
        const res = await fetch("/api/admin/users?page=1&pageSize=100", {
          headers,
        });
        const j = await res.json();
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-brand-gray-50">
                  ...
                  <td className="px-4 py-2 text-brand-gray-500">
                    {user.created_at
                      ...
                      {showModal === "view" && selectedUser && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-auto">
                            ...
                            <button
                              className="mt-6 px-4 py-2 bg-brand-orange text-white rounded"
                      ...
                      {showModal === "invite" && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-auto">
                            ...
                            <button
                              className="mt-4 px-4 py-2 bg-brand-gray-200 text-brand-black rounded"
                              onClick={() => setShowModal(null)}
                            >
                              Cancel
                            </button>
        ...
        <div className="grid gap-6">
          {projects
            ...
            .map((project) => (
              <div
                key={project.id}
                className="bg-brand-white p-6 rounded-lg shadow"
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span
                    className={`px-3 py-1 text-sm rounded-full ${getStatusColor(project.status)}`}
                  >
                    {project.status}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex space-x-2">
                    <button
                      className="px-3 py-1 text-brand-orange hover:bg-brand-orange/5 rounded"
                      onClick={() => {
                    ...
                    <button
                      className="px-3 py-1 text-brand-gray-600 hover:bg-brand-gray-100 rounded"
                      onClick={() => {
                        setEditForm({ ...project });
                        setShowEdit(true);
                      }}
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            if (!res.ok)
                              throw new Error(
                                j.error || "Failed to mark as completed",
                              );
                            await loadProjects();
                          } catch (e) {
                            alert(
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
              <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-auto">
                  ...
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      const headers: HeadersInit = accessToken
                        ...
                        : { "Content-Type": "application/json" };
                      const res = await fetch(
                        `/api/admin/projects/${editForm.id}`,
                        {
                          method: "PATCH",
                          headers,
                ...
                >
                  ...
                  <textarea
                    className="w-full px-3 py-2 border rounded mb-2"
                    placeholder="Project features (long text)"
                  ...
                  <div className="flex gap-2 justify-end">
                    <button
                      type="button"
                      className="px-4 py-2 bg-brand-gray-200 text-brand-black rounded"
                      onClick={() => {
                        setShowEdit(false);
                        setEditForm(null);
                      }}
            ...
            <tbody className="divide-y divide-brand-gray-200">
              {filteredPosts.map((post) => (
                <tr key={post.id} className="hover:bg-brand-gray-50">
                  ...
                  <td className="px-6 py-4 text-brand-gray-500">
                    {post.author}
                  </td>
                  <td className="px-6 py-4 text-brand-gray-500">
                    {post.published_at
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (!res.ok)
                              throw new Error(
                                j.error || "Failed to delete post",
                              );
                            await loadPosts();
                          } catch (e) {
                            alert(
                              (e as any)?.message || "Failed to delete post",
                            );
                          }
                        }}
                      >
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  const headers: HeadersInit = accessToken
                    ...
                    : { "Content-Type": "application/json" };
                  const res = await fetch(
                    `/api/admin/blog-posts/${editForm.id}`,
                    {
                      method: "PATCH",
                      headers,
                      body: JSON.stringify(editForm),
        ...
        <div className="bg-brand-white p-6 rounded-lg shadow">
          ...
          <div className="h-64 bg-brand-gray-100 rounded-lg flex items-center justify-center">
            <p className="text-brand-gray-500">
              Chart placeholder - integrate with analytics service
            </p>
          </div>
        </div>
